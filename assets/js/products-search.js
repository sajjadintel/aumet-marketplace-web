            ...
            columns: [{
                ...
                template: function (row) {
                    if (row.stockUpdateDateTime) {
                        return '<span class="label label-lg font-weight-bold label-inline" style="direction: ltr">' + moment(row.stockUpdateDateTime).format('DD / MM / YYYY') + '</span>';
                    }
                    else {
                        return "";
                    }
                ...
                template: function (row) {
                    if (row.stockStatusId == 1) {
                        return '<div >\
                        <input id="quantity-' + row.id + '" type="text" style="width: 70px; direction: ltr" value="' + row.quantity +
                            '" oninput=\'SearchDataTable.changeProductQuantityCallback(' + JSON.stringify(row) + ' )\' >\
                        <span id="bonus-' + row.id + '" class="label label-xl label-rounded label-primary" style = "width: 50px" > </span></div>';
                    }
                    else {
                        return "";
                    }
                ...
                template: function (row) {
                    ...
                    var btnViewProduct = '<a href="javascript:;" onclick="WebApp.loadSubPage(\'/app/entity/' + row.entityId + '/product/' + row.productId + '\')" class="btn btn-sm btn-default btn-text-primary btn-hover-primary btn-icon mr-2" title="View">\
                    <span class="svg-icon svg-icon-md">\
                    <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px" viewBox="0 0 24 24" version="1.1">\
                    <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">\
                        <rect x="0" y="0" width="24" height="24"/>\
